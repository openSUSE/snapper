...
namespace snapper
{
	...
	CmdMetaHash cmd_metahash(shell_source, SHA256SUM_BIN, SH_BIN,
	                         snapper->getConfig().getSubvolume() +
	                             "/" SNAPSHOTS_NAME);
	...
	for (const ProxySnapshot& source_snapshot : source_snapshots)
	{
	    ...
	    the_big_thing.source_meta_hash = cmd_metahash.get_hash(num);
	...
	CmdMetaHash cmd_metahash(shell_target, backup_config.target_sha256sum_bin, SH_BIN,
	...
	for (const string& num_string : cmd_ls)
	{
		...
		it->target_meta_hash = cmd_metahash.get_hash(it->num);
